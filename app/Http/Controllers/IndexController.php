...
class IndexController extends Controller {
	...
	*/
	...
	public function getListNews($alias) {
		...
		$lang = Session::get('locale');
		$tintuc_cate = NewsCate::where('status', 1)->where('alias', $alias)->first();
		// $tintuc_cate = NewsCate::where('status',1)->where('alias',$alias)->first()->toArray();
		// dd($tintuc_cate);
		if (!empty($tintuc_cate) && $tintuc_cate->com == 'tin-tuc') {
			...
			$com = 'tuyen-dung';
			$data = DB::table('news')->where('cate_id', $tintuc_cate->id)->orderBy('id', 'desc');
			if ($lang == 'vi') {
